...
function getNetworkInfo(retryTimes = 5, retryInterval = 1000) {
  ...
  httpMethod.get('http://ip-api.com/json').then(response => {
    ...
    checkIpRisk(info.query).then((ipInfo) => {
    ...
    if (info.query == '154.17.1.168') { //这里填写vps的IP地址
      getVPSInfo.then((value) => {
        $done({
          ...
          content:
            getIP() +
            `节点IP : ${info.query} (${ipScore}分|${ipRiskCHN}风险)\n` +
            `流　量 :${value.used} /${value.total} (重置:${getRmainingDays(4)}天)\n` +
            `供应商 : ${info.isp}\n` +
            `位　置 : ${info.country} - ${info.city}`,
...
const getVPSInfo = new Promise((resolve, reject) => {
    let opts = {
      ...
      headers: {
        'Cookie': '_ga=GA1.3.382178952.1700617568; _gid=GA1.3.1583462240.1701061700; WHMCSP8D3Nbb7msEy=dfpbbprsvosnosjs87vqub9flk; G_ENABLED_IDPS=google; _gat=1; cf_clearance=QB.Q1KYOgCF7ZhLyfJoY8GEbXHMDRWFipTnzfPuSRjY-1701113356-0-1-7e89bcc.7868844b.50e9c42c-160.0.0; __stripe_mid=55ed9574-2af0-4036-8693-51e59bf4b5078c569c;WHMCSAffiliateID=4007;cus=krystal0798%40gmail.com',
    ...
    $httpClient.get(opts, function (error, response, data) {
      ...
      var match = new Array()
      match[1] = data.match(/Used: ([0-9]{0,4}(\.)?[0-9]{0,4}) (MB|GB)/)
      match[2] = data.match(/Total: ([0-9]{0,4}) GB/)
      if (!match) {
        resolve({ used: '', total: '' })
        return
      }

      let used_full = match[1][0].toString();
      let total_full = match[2][0].toString();
      let used = used_full.substring(5);
      let total = total_full.substring(6);
      //console.log(used)
      //console.log(total)
      resolve({used,total});
    })
  ...
  function checkIpRisk(ip) {
    return checkIpInfo = new Promise((resolve, reject) => {
      let opts = {
        url: 'https://api11.scamalytics.com/用户名/?key=填API密钥&ip='+ip+'&test=0',
        headers: {
